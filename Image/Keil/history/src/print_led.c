...
uint32_t __attribute__((weak)) __div64_32(uint64_t *n, uint32_t base)
{
 ...
 void print_enable(void)
 {
	USART_IO_CONFIG();
	USART1_CLK_ENABLE();
	USART1->US_BRGR = 0x00000028;
	//USART1->US_BRGR = 130; // 20M/130/16=9600
...
//------------------------------------------------------------------------------
 ...
 void print_led(char string_val []) {
    int i;
	  USART1->US_CR = 0X0000000C; 
	  USART1->US_CR = 0X00000040; 
    for(i=0;string_val[i] != '\0';i++) {
			while((USART1->US_CSR & 0X00000002) == 0)
			{};
      USART1->US_THR = string_val[i];
			while((USART1->US_CSR & 0x00000200) == 0x00000200)
		  {};
    }
		for(i = 0;i<5000;i++)
		{
